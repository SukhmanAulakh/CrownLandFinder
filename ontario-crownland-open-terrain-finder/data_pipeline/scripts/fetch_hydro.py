...
def fetch_hydro():
    ...
    layer_config = config.get("waterbodies")
    if not layer_config: return
    
    rest_url = layer_config.get("rest_url")
    logger.info("Fetching OHN Waterbodies...")
    os.makedirs(RAW_DIR, exist_ok=True)
    out_path = RAW_DIR / "waterbodies.geojson"
    ...
    try:
        ...
        logger.error(f"Failed to fetch waterbodies: {e}")
